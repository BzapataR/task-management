...
import { AccessTime, LocationOn, DeleteOutline, EditOutlined } from "@mui/icons-material";
    ...
    >
      <Box sx={{ p: 3 }}>
        ...
        >
          ...
          <Chip
            size="small"
            label={`${event.startTime.format("h:mm A")} - ${event.endTime.format("h:mm A")}`}
            sx={{
              borderRadius: 1,
        ...
        <Box sx={{ display: "flex", justifyContent: "flex-end", gap: 1 }}>
          <Button variant="text" color="error" startIcon={<DeleteOutline />} onClick={() => onDelete(event.id)} size="small">
            Delete
          </Button>
          <Button
            variant="contained"
