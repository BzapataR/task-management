...
export default function EventsPage() {
    ...
    useEffect(() => {
        ...
        if (timeFilter === "upcoming") {
            ...
            result = result.filter(
                (event) =>
                    dayjs().isAfter(event.date) ||
                    (dayjs().isSame(event.date, "day") &&
                        dayjs().isAfter(event.endTime))
            );
        ...
        if (selectedColorFilters.length > 0) {
            result = result.filter((event) =>
                selectedColorFilters.includes(event.color || eventColors[0])
            );
        }
    ...
    const handleColorFilterToggle = (color: string) => {
        if (selectedColorFilters.includes(color)) {
            setSelectedColorFilters(
                selectedColorFilters.filter((c) => c !== color)
            );
        } else {
                                    ...
                                    >
                                        ...
                                        {eventColors.map((color) => (
                                            <Chip
                                                ...
                                                sx={{
                                                    bgcolor: color,
                                                    border: selectedColorFilters.includes(
                                                        color
                                                    )
                                                        ? "2px solid white"
                                                        : "none",
